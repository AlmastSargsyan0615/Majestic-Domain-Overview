...
def navigate_in_new_tab(domain):
    print(domain)
    try:
        ...
        print(f"Data from first URL in new tab: {referring_domains}")
        ...
        print(f"Data from first URL in new tab: {TF_referring_domains}")
...
# Extract domains from the DataFrame
domains = [row[0] for index, row in df.iterrows()]

# Login and open the initial tab
open_and_extract()

# Navigate to different URLs constructed from domains in the new tab
for domain in domains:
    navigate_in_new_tab(domain)
